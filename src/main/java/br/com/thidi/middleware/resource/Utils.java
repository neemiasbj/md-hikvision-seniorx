...
public class Utils {
	...
	public static String convertByteArrayToString(byte[] byteArray, char separator) {
		...
		for (i = (arrayOfByte = byteArray).length, b = 0; b < i;) {
			byte b1 = arrayOfByte[b];
			hexString = String.valueOf(hexString) + String.format("%02X%1", new Object[] { Byte.valueOf(b1) })
					+ separator;
			b++;
		}
	...
	public static String convertByteArrayToString(byte[] byteArray) {
		...
		for (i = (arrayOfByte = byteArray).length, b = 0; b < i;) {
			byte b1 = arrayOfByte[b];
			hexString = String.valueOf(hexString) + String.format("%02X", new Object[] { Byte.valueOf(b1) });
			b++;
		}
	...
	public static String convertByteArrayToHexString(byte[] byteArray) {
		return (byteArray == null) ? null
				: String.join(",", (CharSequence[]) splitStringIntoXStringSize(
						(new BigInteger(1, byteArray)).toString(16).toUpperCase(), Integer.valueOf(2)));
	}

	public static String convertByteArrayToHexString(byte[] byteArray, String strToJoin) {
		if (byteArray == null)
			return null;
		return String.join((strToJoin == null) ? "" : strToJoin,
				(CharSequence[]) splitStringIntoXStringSize((new BigInteger(1, byteArray)).toString(16).toUpperCase(),
						Integer.valueOf(2)));
	}
	...
	public static int calculateBufferChecksum(List<Byte> buffer) {
		int checkSum = 0;
		for (Iterator<Byte> iterator = buffer.iterator(); iterator.hasNext();) {
	...
	public static int calculateBufferChecksum(byte[] buffer) {
		...
		int i;
		byte[] arrayOfByte;
		for (i = (arrayOfByte = buffer).length, b = 0; b < i;) {
			byte b1 = arrayOfByte[b];
			checkSum += b1 & 0xFF;
			b++;
		}
